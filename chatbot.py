...
chats = open('dataset.txt', encoding = 'utf-8', errors = 'ignore').read().split('+')
...
for chat in chats[:-1]:
    _chat = chat.split('\n')
    if chat in chats[0]:
        questions.append(_chat[0])
        answers.append(_chat[1])
    else:
        questions.append(_chat[1])
        if len(_chat) == 3:
            answers.append(_chat[2])
        if len(_chat) >3:
            answers.append(",".join(_chat[2:]))
...
#creating 2 dict that maps the questions words and the answers word to a unique integer
...
#Translate all the questions and answers into unique integers 
...
while(True):
    ...
    question = question + [questionsword2int['<PAD>']] * (25 - len(question))
